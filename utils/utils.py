...
def calculate_fid(x_1,x_2, name):
...
def calculate_mahalanobis_of_samples (x1, x2):
    A_flat = x1.flatten()
    B_flat = x2.flatten()
    combined = np.vstack([A_flat, B_flat])
    cov_matrix = np.cov(combined, rowvar=False)
    epsilon = 1e-10
    inv_cov_matrix = np.linalg.inv(cov_matrix + epsilon * np.eye(cov_matrix.shape[0]))
    distance = mahalanobis(A_flat, B_flat, inv_cov_matrix)
    return distance
